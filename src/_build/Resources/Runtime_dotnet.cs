...
{
    public class Runtime_dotnet : IResource
    {
        public string Id => "dotnet";

        public string Name => ".NET运行库";
        ...
        private string getDotNetRuntimeAspNetCoreDownloadUrl(HttpClient client, string dotnetVersion, string arch)
        {
            var url = $"https://dotnet.microsoft.com/zh-cn/download/dotnet/thank-you/runtime-aspnetcore-{dotnetVersion}-{arch}-binaries";
            var html = client.GetStringAsync(url).Result;
            var document = new HtmlAgilityPack.HtmlDocument();
            document.LoadHtml(html);
        ...
        public void Invoke()
        {
            ...
            foreach (var arch in selectArchs)
            {
                ...
                var url = getDotNetRuntimeAspNetCoreDownloadUrl(client, dotnetVersion, archDict[arch]);
