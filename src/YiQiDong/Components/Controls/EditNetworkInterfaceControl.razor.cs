...
namespace YiQiDong.Components.Controls
{
    public partial class EditNetworkInterfaceControl
    {
        ...
        private NetworkInterfaceConfig GetNetworkInterfaceConfig(DisplayNetworkInterfaceInfo model)
        {
            ...
            {
                config = new NetworkInterfaceConfig() { ActiveMetohd = ActiveMethod.None };
                ...
                await Task.Run(async () =>
                {
                    ...
                    else
                    {
                        ...
                        if (ActiveMethodConfig != null && CurrentNetworkInterfaceConfig.ActiveMetohd != ActiveMethod.None)
                        {
                            ...
                            sb.Append(CurrentNetworkInterface.Name);
                        ...
                        if (IPv4Config != null)
                        {
                            ...
                            using (var writer = new StreamWriter(fs))
                            {
                                ...
                                {
                                    //本网卡部分
                                    writer.WriteLine($"iface {CurrentNetworkInterface.Name} inet {CurrentNetworkInterfaceConfig.IPv4Config.Method.ToString().ToLower()}");
                                    if (CurrentNetworkInterfaceConfig.IPv4Config.Method == NetworkInterfaceMethod.Static)
                                    {
                                        if (!string.IsNullOrEmpty(CurrentNetworkInterfaceConfig.IPv4Config.IPAddress))
                                            writer.WriteLine($"address {CurrentNetworkInterfaceConfig.IPv4Config.IPAddress}");
                                        if (!string.IsNullOrEmpty(CurrentNetworkInterfaceConfig.IPv4Config.NetMask))
                                            writer.WriteLine($"netmask {CurrentNetworkInterfaceConfig.IPv4Config.NetMask}");
                                        if (!string.IsNullOrEmpty(CurrentNetworkInterfaceConfig.IPv4Config.Gateway))
                                            writer.WriteLine($"gateway {CurrentNetworkInterfaceConfig.IPv4Config.Gateway}");
                                        if (!string.IsNullOrEmpty(CurrentNetworkInterfaceConfig.IPv4Config.DnsServer))
                                            writer.WriteLine($"dns-nameserver {CurrentNetworkInterfaceConfig.IPv4Config.DnsServer}");
                                    }
                                }
                                //配置后面
                        ...
                        if (IPv6Config != null)
                        {
                            ...
                            using (var writer = new StreamWriter(fs))
                            {
                                ...
                                {
                                    //本网卡部分
                                    writer.WriteLine($"iface {CurrentNetworkInterface.Name} inet6 {CurrentNetworkInterfaceConfig.IPv6Config.Method.ToString().ToLower()}");
                                    if (CurrentNetworkInterfaceConfig.IPv6Config.Method == NetworkInterfaceMethod.Static)
                                    {
                                        if (!string.IsNullOrEmpty(CurrentNetworkInterfaceConfig.IPv6Config.IPAddress))
                                            writer.WriteLine($"address {CurrentNetworkInterfaceConfig.IPv6Config.IPAddress}");
                                        if (!string.IsNullOrEmpty(CurrentNetworkInterfaceConfig.IPv6Config.NetMask))
                                            writer.WriteLine($"netmask {CurrentNetworkInterfaceConfig.IPv6Config.NetMask}");
                                        if (!string.IsNullOrEmpty(CurrentNetworkInterfaceConfig.IPv6Config.Gateway))
                                            writer.WriteLine($"gateway {CurrentNetworkInterfaceConfig.IPv6Config.Gateway}");
                                        if (!string.IsNullOrEmpty(CurrentNetworkInterfaceConfig.IPv6Config.DnsServer))
                                            writer.WriteLine($"dns-nameserver {CurrentNetworkInterfaceConfig.IPv6Config.DnsServer}");
                                    }
                                }
                                //配置后面
